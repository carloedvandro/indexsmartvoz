...
export function PlanCarousel({ plans, onSelectPlan, isMobile }: PlanCarouselProps) {
  const getPlanTypeLabel = (gb: string) => {
    switch (gb) {
      case "80GB": return "Basic Plan";
      case "100GB": return "Standard Plan";
      case "120GB": return "Premium Plan";
      case "140GB": return "Pro Plan";
      default: return "Plan";
    }
  };
        ...
        <CarouselContent className={isMobile ? "-ml-2 md:-ml-4" : "-ml-4"}>
          {plans.map((plan, index) => (
            <CarouselItem 
              ...
              className={
                isMobile 
                  ? "pl-2 md:pl-4 basis-[85%] sm:basis-1/2" 
