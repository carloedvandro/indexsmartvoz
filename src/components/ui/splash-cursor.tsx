...
// Define TypeScript interfaces for the WebGL classes
...
// Custom types for WebGL formats to handle both WebGL1 and WebGL2
interface GLFormat {
  internalFormat: number;
  format: number;
}

// Custom type for the extended WebGL context with format information
interface WebGLContextWithExt {
...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function pointerPrototype(this: PointerProto) {
      this.id = -1;
      this.texcoordX = 0;
      this.texcoordY = 0;
    ...
    // @ts-ignore - This is necessary for the custom constructor
    ...
    function getWebGLContext(canvas: HTMLCanvasElement): WebGLContextWithExt {
      const params = {
        alpha: true,
        depth: false,
        stencil: false,
      ...
      if (isWebGL2) {
        ...
        supportLinearFiltering = webGL1.getExtension(
    ...
    function drawDisplay(target: FBO | null) {
      ...
      blit(target || undefined);
    }

    function splatPointer(pointer: PointerProto) {
      let dx = pointer.deltaX * config.SPLAT_FORCE;
      let dy = pointer.deltaY * config.SPLAT_FORCE;
      splat(pointer.texcoordX, pointer.texcoordY, dx, dy, pointer.color);
    }

    function clickSplat(pointer: PointerProto) {
    ...
    function updatePointerDownData(pointer: PointerProto, id: number, posX: number, posY: number) {
      pointer.id = id;
      pointer.down = true;
      pointer.moved = false;
    ...
    function updatePointerMoveData(pointer: PointerProto, posX: number, posY: number, color: { r: number; g: number; b: number }) {
      pointer.prevTexcoordX = pointer.texcoordX;
      pointer.prevTexcoordY = pointer.texcoordY;
      pointer.texcoordX = posX / canvas.width;
      pointer.texcoordY = 1.0 - posY / canvas.height;
    ...
    function updatePointerUpData(pointer: PointerProto) {
      pointer.down = false;
    }
