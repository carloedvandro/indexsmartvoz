    ...
    duration: 0.5
      ...
      <div className="mb-5 relative bg-transparent">
        ...
        <div className="flex justify-center gap-2 mt-3">
          {plans.map((_, index) => <button key={index} onClick={() => scrollToCard(index)} className={`w-3 h-3 rounded-full transition-colors duration-300 ${index === currentPlan ? 'bg-primary' : 'bg-gray-300'}`} />)}
        </div>
      </div>
      ...
      duration: 0.3
        ...
        {plan.commissionLevels.map((level, index) => <div key={level.level} className="w-full md:w-[48%] bg-white rounded-2xl p-5 pl-6" style={{
      ...
      duration: 0.3,
      delay: 0.1
    }} className="bg-white p-6 rounded-2xl mt-5" style={{
      boxShadow: '0 6px 16px rgba(0,0,0,0.09)'
        ...
        <div className="overflow-x-auto mt-3">
          <table className="w-full border-collapse">
            ...
            <tbody>
              {plan.commissionLevels.map(level => <tr key={level.level}>
