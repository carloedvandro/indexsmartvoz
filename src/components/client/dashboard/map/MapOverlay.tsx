...
export function MapOverlay({ regionsData, activeRegion, setActiveRegion }: MapOverlayProps) {
  return (
    <div className="absolute inset-0 w-full h-full">
      ...
      {/* Sudeste - área vermelha centro-direita - movida 40px para baixo e 5px para direita no mobile */}
      <motion.div
        className="absolute cursor-pointer"
        style={{
          top: 'calc(50% - 80px)', // Movida 40px para baixo (de -120px para -80px)
          right: 'calc(15% + 40px)', // Movida 5px para direita (de +45px para +40px)
          width: '40%',
          height: '25%',
        }}
        whileHover={{ scale: 1.02 }}
      ...
      {Object.entries(regionsData).map(([key, region], index) => {
        const positions = {
          ...
          sudeste: { top: 'calc(62% - 80px)', right: 'calc(23% + 40px)', transform: 'translate(50%, -50%)' }, // Ajustado para acompanhar a região
