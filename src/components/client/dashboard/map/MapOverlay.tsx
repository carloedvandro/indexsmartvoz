...
export function MapOverlay({ regionsData, activeRegion, setActiveRegion }: MapOverlayProps) {
  return (
    <div className="absolute inset-0 w-full h-full">
      ...
      {/* Nordeste - área azul direita - movida 4,5px para baixo e 1,5px para esquerda */}
      <motion.div
        className="absolute cursor-pointer"
        style={{
          top: 'calc(25% + 4.5px)',
      ...
      {/* Centro-Oeste - área laranja centro - movida 5px para cima e 3px para esquerda */}
      <motion.div
        className="absolute cursor-pointer"
        style={{
          top: '35%',
      ...
      {Object.entries(regionsData).map(([key, region], index) => {
        const positions = {
          ...
          nordeste: { top: 'calc(42% + 4.5px)', right: 'calc(23.5% + 1.5px)', transform: 'translate(50%, -50%)' },
          centrooeste: { top: '47%', left: '50%', transform: 'translate(-50%, -50%)' },
