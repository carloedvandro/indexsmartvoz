    ...
    <div className="w-full">
      ...
      <div className="w-full">
        {dueDates.map((row, rowIndex) => (
          <div key={rowIndex} className="grid grid-cols-4 gap-4 w-full mb-4">
            {row.map((date) => (
              <Card 
                key={date}
                className={`cursor-pointer transition-all duration-200 h-10 flex items-center justify-center shadow-none relative
