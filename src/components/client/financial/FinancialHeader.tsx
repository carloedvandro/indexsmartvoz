
export function FinancialHeader() {
  return (
    <div className="fixed top-0 left-0 right-0 h-16 bg-[#46005e] border-b border-white/10 z-50 relative">
      <div className="h-full flex items-center px-6 relative z-10">
        <div className="flex flex-col">
          <h1 className="text-sm text-gray-400 font-normal leading-tight">Financeiro</h1>
          <h2 className="text-xl text-white font-medium leading-7">Resumo</h2>
        </div>
      </div>
      <div className="absolute inset-0 bg-black/20" />
    </div>
  );
}
