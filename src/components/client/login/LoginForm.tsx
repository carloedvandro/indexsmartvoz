...
export function LoginForm({
  ...
  return <motion.form onSubmit={handleSubmit} variants={containerVariants} className="space-y-5 w-full mt-[40px] mx-auto px-0">
      ...
      <motion.div className="space-y-4" variants={itemVariants}>
        <div className="relative w-full">
          <Input id="password" type={showPassword ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} onFocus={() => setPasswordFocused(true)} onBlur={() => setPasswordFocused(false)} required className="w-full pr-15 bg-white border-2 border-[#7a1fa2] rounded-md h-12 text-black focus:max-w-full " />
