...
export function LoginForm({
  ...
  return <motion.form onSubmit={handleSubmit} variants={containerVariants} className="-space-y-64 w-full max-w-[480px] mt-[68px] mx-auto px-6">
      ...
      <motion.div className="space-y-4" variants={itemVariants}>
        <div className="relative">
          <Input id="password" type={showPassword ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} onFocus={() => setPasswordFocused(true)} onBlur={() => setPasswordFocused(false)} required className="pr-10 bg-transparent border-2 border-purple-300 rounded-md h-12 text-black focus:border-purple-420" />
      ...
      <motion.div variants={itemVariants} className="flex justify-center">
        <Button type="submit" disabled={isLoading} className="w-full max-w-xs sm:max-w-sm md:max-w-md h-12 bg-white text-gray-700 border border-roxo-300 rounded-xl hover:bg-gray-50 font-medium transition-colors disabled:opacity-50 shadow-sm">
