...
export function CashbackModal({
  ...
  useEffect(() => {
    if (open) {
      if (initialData) {
        const type = initialData.amount !== undefined && initialData.amount !== null ? 'fixed' : 'percentage';
        setValueType(type);
        reset({
          level: initialData.level,
          valueType: type,
          percentage: type === 'percentage' ? initialData.percentage : undefined,
          amount: type === 'fixed' ? initialData.amount : undefined,
  ...
  const handleFormSubmit = (data: CashbackFormData) => {
    const submitData = {
      ...
      amount: data.valueType === 'fixed' ? data.amount : null,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
          ...
          {valueType === 'fixed' && (
            <div>
              <Label htmlFor="amount">Valor Fixo (R$) *</Label>
              <Input 
                id="amount" 
                type="number" 
                step="0.01" 
